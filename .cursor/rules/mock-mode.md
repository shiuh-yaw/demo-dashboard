...
This app has a **mock mode** toggle (wallet dropdown). When enabled, actions are mocked instead of executed on-chain. Mock data is stored in Dynamic user metadata under keys: `trade`, `earn`, `predict`.
...
3. **Mock the action** so that when `useMockMode().isMockMode` is true, it:
   ...
   - Calls `useUpdateMetadata().mutateAsync()` with the new data merged into the correct metadata key
   - Returns a mock success value (e.g. `"mock-tx"`)
4. **Add a "My X" section** that:
   - Renders only when `isMockMode` is true and there is data
   - Reads from `metadata[MOCK_METADATA_KEYS.TRADE]` or `metadata[MOCK_METADATA_KEYS.PREDICT]` via `useUserMetadata()`
...
- **Metadata update API:** `POST /api/user/metadata/update` with body `{ metadata: Record<string, unknown> }`
...
- **Metadata hook:** `useUserMetadata()` and `useUpdateMetadata()` from `@/hooks/`
