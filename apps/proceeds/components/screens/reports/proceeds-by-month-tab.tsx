    ...
    <>
      ...
      <div className="flex items-center justify-between gap-4 mb-5">
        <button
          ...
          className="w-8 h-8 rounded-md flex items-center justify-center text-(--widget-muted) hover:text-(--widget-fg) hover:bg-(--widget-row-bg) transition-colors disabled:opacity-40 disabled:hover:bg-transparent"
        ...
        {hasAnyDemoRecord && (
          <button
            ...
            className="inline-flex items-center gap-1.5 text-[13px] font-medium text-(--widget-muted) hover:text-(--widget-fg) transition-colors"
      ...
      <div className="mb-2 mt-8 flex items-baseline justify-between">
        <h3 className="text-[15px] font-semibold text-(--widget-fg)">
  ...
  return (
    <div className="relative">
      ...
      <button
        ...
        aria-expanded={open}
        className="text-[18px] font-semibold text-(--widget-fg) tracking-tight inline-flex items-center gap-1.5 hover:text-(--widget-primary) transition-colors"
      >
        {month.month}
      ...
      {open && (
        <div
          role="listbox"
          className="absolute top-full left-1/2 -translate-x-1/2 mt-2 z-20 min-w-[220px] bg-(--widget-bg) border border-(--widget-border) rounded-lg overflow-hidden shadow-lg"
        >
          {months.map((m, i) => (
            <button
              key={m.monthKey}
              ...
              onClick={() => onSelect(i)}
              className="w-full flex items-center justify-between text-left px-3.5 py-2.5 hover:bg-(--widget-row-bg) transition-colors"
              style={{
                background:
                  i === monthIndex ? "var(--widget-row-bg)" : "transparent",
              }}
            >
              <span className="text-[13px] text-(--widget-fg)">{m.month}</span>
              <span className="text-[12px] text-(--widget-muted) tabular-nums">
      ...
      <table className="data-table">
          ...
          {month.breakdown.map((row) => (
            <tr key={row.countryCode}>
              ...
              <td style={{ textAlign: "right" }}>
                <span className="text-sm text-(--widget-fg) tabular-nums">
                  {formatUnits(row.units)}
                </span>
              </td>
              <td style={{ textAlign: "right" }}>
                <span className="text-sm text-(--widget-muted) tabular-nums whitespace-nowrap">
                  {row.localCurrency}{" "}
              ...
              <td style={{ textAlign: "right" }}>
                <span className="text-sm text-(--widget-muted) tabular-nums">
                  {formatFxRate(row.fxRate)}
                </span>
              </td>
              <td style={{ textAlign: "right" }}>
                <span className="text-sm font-medium text-(--widget-fg) tabular-nums whitespace-nowrap">
        ...
        <tfoot>
          <tr style={{ borderTop: "1px solid var(--widget-border)" }}>
            ...
            >
              <span className="text-[15px] text-(--widget-fg) tabular-nums whitespace-nowrap">
