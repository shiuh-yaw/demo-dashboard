 ...
 *
 * Only the tokens trade personalizes per brand are emitted; everything
 * else falls through to the static `--brand-*` defaults in
 * `app/globals.css` (which carry trade's own design language) and to
 * `@dynamic-demos/theme/defaults.css` below that. Zero FOUC,
 * zero hydration mismatch — the inline `<style>` beats client paint.
 *
 * Mirror of `apps/wallet/lib/wallet-brand.ts` (D-008 unified pattern).
 */
