...
export type Threat = "none" | "rimau-db" | "dynamic-infra" | "insider";
...
export function KeySplitDiagram({ wallet, external, threat, person }: { wallet: SessionWallet; external: ExternalWallet | null; threat: Threat; person: Person | null }) {
  ...
  const hitRimau = threat === "rimau-db" || threat === "insider";
  ...
  return (
    <svg viewBox="0 0 960 560" className="w-full h-auto" role="img" aria-label="2-of-2 TSS-MPC key split for this wallet">
      ...
      {/* Device */}
      <Node x={40} y={110} w={240} h={230} title={`User's device ${wallet.deviceId}`} subtitle={person?.name ?? "Rimau user"} accent={BRAND} dim={threat !== "none"}>
      ...
      {/* Enclave */}
      <Node x={680} y={110} w={240} h={230} title="Dynamic · hardware enclave" subtitle="Trusted execution environment" accent={ENCLAVE} hit={hitDynamic} dim={threat === "rimau-db" || threat === "insider"}>
      ...
      {/* Rimau backend */}
      <Node x={300} y={380} w={360} h={150} title="Rimau Exchange backend" subtitle="Their platform team's stack" accent={INK} hit={hitRimau}>
        <text x="316" y="440" fontSize="12" fill={INK}>
          <tspan x="316" dy="0">Holds: user records · session tokens · positions ledger</tspan>
        </text>
        <rect x="316" y="456" width="328" height="34" rx="8" fill={hitRimau ? "#fdecec" : "#e7f6ee"} />
        <text x="480" y="478" textAnchor="middle" fontSize="13" fontWeight="700" fill={hitRimau ? DOWN : "#1f8a5b"}>
      ...
      {threat !== "none" && (
        <g>
          ...
          <text x="480" y="42" textAnchor="middle" fontSize="13" fontWeight="700" fill={DOWN}>
            {threat === "rimau-db" && "Attacker inside Rimau's database"}
            {threat === "dynamic-infra" && "Attacker inside Dynamic's infrastructure, outside the enclave"}
            {threat === "insider" && "Malicious insider at Rimau"}
          </text>
