      ...
      <div className="space-y-6">
        {justRestored && (
          <div className="rise flex items-center gap-3 rounded-2xl bg-info-2 text-info px-4 py-3 text-[14px]">
            <Icon.Shield />
            <span>
              <strong>Restored on this device.</strong> Same account, same balance, same positions. Nothing was typed, nothing was held by Rimau.
            </span>
          </div>
        )}
        ...
        <Card className="p-0 overflow-hidden">
          <div className="px-5 py-4 border-b border-line flex items-center justify-between">
            ...
            <span className="text-[12px] text-muted">Held in your Rimau account, keys under your control</span>
          </div>
          <ul>
            ...
            <Row
              ...
              note={state.balances.eth === 0 ? "You don't need ETH. Rimau covers network fees on your transfers." : undefined}
            />
          </ul>
        </Card>
        ...
        <Card>
          ...
          <div className="mt-4 grid sm:grid-cols-2 gap-3">
            <WalletTile label="Rimau account" sub="Created with your sign-in · 2-of-2 · keys never leave your control" address={wallet.address} revealed={state.revealAddress} active />
            {state.external ? (
              ...
              <div className="rounded-xl border border-dashed border-line p-4 text-[13px] text-muted">
                Already have a wallet? Connect it and keep your DeFi activity inside Rimau: same session, same limits, same support.
              </div>
            )}
          </div>
        </Card>
...
function MarketsStrip() {
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
      {rows.map((r) => (
        <Link key={r.sym} href="/markets" className="rimau-card p-3.5 text-left hover:bg-ground/60 transition-colors block">
