...
import { RIMAU_SDK_STEPS } from "../lib/code-steps";

describe("code steps", () => {
  it("every TypeScript snippet opens with its import line", () => {
    for (const step of RIMAU_SDK_STEPS) {
  ...
  it("numbers are unique and docs links are https", () => {
    const nums = RIMAU_SDK_STEPS.map((s) => s.num);
    expect(new Set(nums).size).toBe(nums.length);
    for (const s of RIMAU_SDK_STEPS) expect(new URL(s.docsUrl).protocol).toBe("https:");
  });
});
