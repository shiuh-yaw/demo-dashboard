 ...
 *
 * Only the tokens deposit personalizes per brand are emitted; everything
 * else falls through to the static `--brand-*` defaults in
 * `app/globals.css` (deposit's own design language) and to
 * `@dynamic-demos/theme/defaults.css` below that. Zero FOUC,
 * zero hydration mismatch — the inline `<style>` beats client paint.
 *
 * Mirrors `apps/wallet/lib/wallet-brand.ts` — deposit's `flow_role` is
 * `wallet` (per its AGENTS.md), so it shares the wallet config shape.
 */
import {
  darkenHex,
  type BrandTheme,
