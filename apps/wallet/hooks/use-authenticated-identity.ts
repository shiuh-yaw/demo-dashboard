 ...
 * verified email) across every auth method. Mirrors use-auth's event
 * subscription so the value flows in when the SDK populates the user, which
 * can lag the `isSignedIn` flip (e.g. social login).
 */
