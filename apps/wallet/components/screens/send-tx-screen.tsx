...
export function SendTxScreen({
  ...
  const selectedToken = useMemo((): TokenBalanceInfo | null => {
    ...
    if (tokenBalances.length === 1) return tokenBalances[0] ?? null;
    return tokenBalances.find((t) => t.isNative) ?? tokenBalances[0] ?? null;
