...
export function useCompleteSocialAuth(
  ...
  useEffect(() => {
    ...
    async function handleOAuthRedirect() {
      try {
        // Initialize Dynamic client first
        await initializeDynamic();
        await waitForClientInitialized();

        // Helper to handle successful auth - syncs cookie and triggers redirect
        // Does NOT set isLoading to false to prevent login button flash
        // NOTE: Wallet creation is handled by DynamicInit, not here (avoids race conditions)
        const handleAuthSuccess = async () => {
          ...
          // Redirect - keep isLoading true to prevent flash
          onSuccess?.();
        };
        ...
        if (!isReturning) {
          ...
          // Only do retries if we had OAuth params
          ...
          if (hadOAuthParams) {
            ...
            // No OAuth params - just a quick auth check (no retries needed)
            if (checkAuthStatus()) {
