...
interface CreditCardProps {
  ...
  onAddFunds?: () => void;
  /** Show the app logo from branding config */
  showLogo?: boolean;
  fullCardNumber?: string; // Full unmasked card number for reveal
}
...
export const CreditCard = ({
  ...
  // Get the display card number (full or masked)
  const displayCardNumber = useMemo(() => {
    if (showCardDetails && fullCardNumber) {
      return fullCardNumber;
    }
    // If cardNumber is already masked, replace periods with bullet points
    if (typeof cardNumber === "string" && cardNumber.includes("....")) {
      return cardNumber.replace(/\./g, "•");
    }
    // Otherwise mask it with bullet points
    if (cardNumber) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Top Section: Branding and Action Button */}
        <div
        ...
        >
          <div className="flex items-center gap-2">
            {showLogo && (
              <div
                className={cn(
                  "shrink-0",
                  // Some logos have internal padding that needs margin compensation
                  branding.logo === "youtube" && "-ml-3.5",
                )}
              >
                <AppLogo
                  className={cn(
                    "h-3.5",
                    type === "gray-light" || type === "brand-light"
                      ? ""
                      : "brightness-0 invert",
                  )}
                  brand={branding.logo}
          ...
          {showAddFundsButton && onAddFunds && (
            <button
              ...
              className={cn(
                "px-3 py-1.5 rounded-md text-xs font-medium transition-all duration-200 shrink-0 shadow-sm",
                type === "gray-light" || type === "brand-light"
                  ? "bg-white text-gray-900 hover:bg-gray-50 border border-gray-200 hover:shadow-md"
                  : "bg-white/20 text-white hover:bg-white/30 border border-white/30 backdrop-blur-sm hover:shadow-md",
              )}
            >
              Add funds
            </button>
        ...
        {/* Middle Section: Balance (if provided) */}
        {balance && (
          <div className="relative flex items-start pl-0 pr-1 pt-6">
            <div
              className={cn(
            ...
            <div className="flex items-end gap-3">
              {cardCvv && (
                <div
                  ...
                  className={cn(
                    "text-xs sm:text-sm font-medium tracking-wider uppercase gap-1 flex items-center leading-tight",
                    styles[type].footerText,
                  )}
                >
                  <span className={cn("opacity-60", styles[type].footerText)}>
                    CVV
                  </span>
                  {showCardDetails ? (
                    <>
              ...
              {cardExpiration && (
                <div
                  className={cn(
                    "text-xs sm:text-sm font-medium tracking-wider tabular-nums gap-1 flex items-center leading-tight",
                    styles[type].footerText,
                  )}
                >
                  <span className={cn("opacity-60", styles[type].footerText)}>
