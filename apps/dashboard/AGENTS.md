...
- `/api/share/context?token=` (Phase GTM-05) - public, CORS-allowlisted (`TRACK_CORS_ORIGINS`) + `OPTIONS`; resolves an active share token to `{ prospectName, cta: { label, url } | null }` (never emails, ids, or theme internals - see `lib/share-links/context.ts`), `{}` for anything invalid/missing/inactive. Always 200, `Cache-Control: no-store`. Ingest (`POST /api/track`) is Phase GTM-06.
...
- `TRACK_CORS_ORIGINS` - comma-separated EXTERNAL demo origins allowlisted for the public tracker endpoints (`/api/share/context`, Phase GTM-06's `/api/track`). https `dynamic.dev` + subdomains are always allowed (built in); this var is only for demos hosted elsewhere. Empty (default) = builtin origins only.
