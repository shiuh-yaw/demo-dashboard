/**
 * Pure functions that normalise legacy records into ProspectSeed values.
 * Each extractor is total — invalid input returns null + an emitted
 * "skipped" record that the orchestrator surfaces in the report.
 *
 * Phase 2-brand-cutover (2026-05-06): the ProspectProfile extractor now
 ...
 * full ProspectTheme; their seeds set the new fields to null and the
...
import type { ProspectBorderRadius, ProspectLogoKind } from "@/lib/services/types";
import { isHexColor, normaliseHex } from "./hash";
import type { ProspectSeed, ProspectSource } from "./types";
...
const BORDER_RADIUS_VALUES: ReadonlySet<ProspectBorderRadius> = new Set([
  "xs",
  "sm",
  "md",
  "lg",
...
function asBorderRadius(value: unknown): ProspectBorderRadius | null {
  if (typeof value !== "string") return null;
  return BORDER_RADIUS_VALUES.has(value as ProspectBorderRadius)
    ? (value as ProspectBorderRadius)
    : null;
}

function asLogo(value: unknown): ProspectLogoKind {
 ...
 * prospects. Pass non-empty strings through verbatim; null otherwise.
 */
...
interface BuildSeedArgs {
  ...
  /** Optional theme overlay for richer extraction (ProspectProfile only). */
  theme?: Partial<Record<keyof ProspectTheme, unknown>>;
  /** Optional theme-radius (legacy may also carry it on ProspectSettings). */
  borderRadius?: unknown;
  logo?: unknown;
  logoUrl?: string | null;
  companyUrl?: string | null;
  /** Mirror the ProspectProfile.demos linkage onto the seed. */
...
export function extractFromProspectProfile(
  profile: ProspectProfile,
): ExtractResult {
  const settings = profile.prospect;
  const theme: Partial<ProspectTheme> = settings.theme ?? {};
  ...
  return buildSeed({
    ...
    theme: theme as Partial<Record<keyof ProspectTheme, unknown>>,
    borderRadius: radius,
    logo: settings.logo,
    logoUrl,
...
export function extractFromCheckout(
  ...
  // pick the prospect-relevant fields without dragging the full type in.
