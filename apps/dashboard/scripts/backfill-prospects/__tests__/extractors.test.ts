...
const baseProfile = (over: Partial<ProspectProfile> = {}): ProspectProfile => ({
  id: "bp_1",
  name: "Acme",
  prospect: {
    logo: "custom",
...
describe("extractFromProspectProfile", () => {
  it("emits a seed with normalised primary color and original logo", () => {
    const { seed, skipReason } = extractFromProspectProfile(baseProfile());
    ...
    expect(seed!.source).toEqual({ kind: "prospect-profile", id: "bp_1" });
  });

  it("falls back to ProspectSettings.primaryColor when theme is absent", () => {
    const profile = baseProfile();
    profile.prospect.theme = undefined;
    const { seed } = extractFromProspectProfile(profile);
    expect(seed!.primaryColor).toBe("#ff0000");
  });
  ...
  it("skips when primaryColor is not a hex string", () => {
    ...
    broken.prospect.primaryColor = "rgba(255,0,0,0.5)" as unknown as string;
    broken.prospect.theme = undefined;
    const { seed, skipReason } = extractFromProspectProfile(broken);
    expect(seed).toBeNull();
...
// =============================================================================
// Phase 2-brand-cutover: extended coverage for the wider Prospect row.
// =============================================================================

describe("Phase 2-brand-cutover extractor coverage", () => {
  function fullProfile(overrides: Partial<ProspectProfile> = {}): ProspectProfile {
    return {
      id: "bp_1",
      name: "Acme",
  ...
  it("extracts every visual theme field from a ProspectProfile aggregate", () => {
    const { seed, skipReason } = extractFromProspectProfile(fullProfile());
    expect(skipReason).toBeUndefined();
    expect(seed).not.toBeNull();
  ...
  it("falls back to ProspectSettings convenience accessors when theme is missing", () => {
  ...
  it("idempotency invariant: hash unchanged when ownerId/primaryColor/logoUrl stay the same", async () => {
    const { hashProspectKey } = await import("../hash");
    const a = extractFromProspectProfile(fullProfile()).seed!;
    const b = extractFromProspectProfile(
      fullProfile({
        // Keep ownerId, primaryColor, logoUrl identical; mutate
        // everything else. Hash MUST match.
        demos: { earn: "different_earn" },
        prospect: {
          ...fullProfile().prospect,
          theme: {
            ...fullProfile().prospect.theme!,
            pageBackground: "#000000",
          },
        },
      }),
    ).seed!;
    expect(hashProspectKey(a)).toBe(hashProspectKey(b));
  });
