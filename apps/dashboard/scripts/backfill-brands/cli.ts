 ...
 *
 * Wires real Redis and the BrandService selected by USE_POSTGRES_BRANDS,
 ...
 *
 * Usage:
 *   USE_POSTGRES_BRANDS=true pnpm --filter @dynamic-demos/dashboard backfill:brands
 *
 * Exit codes:
 ...
 *   1 — fatal error reaching Redis or the brand service.
 *
...
main().catch((err) => {
  ...
  process.stderr.write(
    `[backfill-brands] fatal: ${err instanceof Error ? err.message : String(err)}\n`,
  );
  process.exitCode = 1;
});
