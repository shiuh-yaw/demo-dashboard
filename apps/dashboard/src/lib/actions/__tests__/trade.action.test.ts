/**
 * Trade list wiring: getAllTradeConfigs must resolve the ACTIVE My/Team/All
 * scope (team switcher x filter) and filter via demoConfigActiveScopeWhere -
 * never the broad get()-authorization demoConfigVisibilityWhere. Mirrors
 * wallets.action.test.ts's list-wiring describe.
 */
...
vi.mock("@/lib/services/demo-config-mappers/trade", () => ({
  tradeMapper: {
...
describe("getAllTradeConfigs (list wiring, active scope not broad visibility)", () => {
  it("resolves the active scope and scopes the DB query via demoConfigActiveScopeWhere + kind", async () => {
    ...
    const { configs } = await getAllTradeConfigs();
    ...
    expect(configs.map((c) => (c as { id: string }).id).sort()).toEqual(["mine", "theirs"]);
  });
