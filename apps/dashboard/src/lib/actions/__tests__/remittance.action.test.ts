/**
 * Remittance list wiring: getAllRemittanceConfigs must resolve the ACTIVE
...
vi.mock("@/lib/services/demo-config-mappers/remittance", () => ({
  remittanceMapper: {
...
describe("getAllRemittanceConfigs (list wiring, active scope not broad visibility)", () => {
  it("resolves the active scope and scopes the DB query via demoConfigActiveScopeWhere + kind", async () => {
    ...
    const { configs } = await getAllRemittanceConfigs();
