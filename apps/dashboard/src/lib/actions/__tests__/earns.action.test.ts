/**
 * Earn list wiring: getAllEarnConfigs must resolve the ACTIVE My/Team/All
...
vi.mock("@/lib/services/demo-config-mappers/earn", () => ({
  earnMapper: {
...
describe("getAllEarnConfigs (list wiring, active scope not broad visibility)", () => {
  it("resolves the active scope and scopes the DB query via demoConfigActiveScopeWhere + kind", async () => {
    ...
    const { configs } = await getAllEarnConfigs();
