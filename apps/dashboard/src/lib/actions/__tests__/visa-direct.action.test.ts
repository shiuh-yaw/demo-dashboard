/**
 * Visa Direct list wiring: getAllVisaDirectConfigs must resolve the ACTIVE
...
vi.mock("@/lib/services/demo-config-mappers/visa-direct", () => ({
  visaDirectMapper: {
...
describe("getAllVisaDirectConfigs (list wiring, active scope not broad visibility)", () => {
  it("resolves the active scope and scopes the DB query via demoConfigActiveScopeWhere + kind", async () => {
    ...
    const { configs } = await getAllVisaDirectConfigs();
