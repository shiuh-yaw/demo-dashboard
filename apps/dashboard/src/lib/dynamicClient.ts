 ...
 *
 * Redirects user to Google OAuth page. After authentication,
 * user is redirected back to the same page where the
 * useCompleteSocialAuth hook will complete the flow.
 *
 * @param redirectUrl - Optional custom redirect URL (defaults to current page)
 */
export async function signInWithGoogle(redirectUrl?: string): Promise<void> {
  const client = getClient();
  if (!client) {
    throw new Error(
      environmentId
        ? "Dynamic client not initialized"
        : "NEXT_PUBLIC_DYNAMIC_ENVIRONMENT_ID is required",
    );
  }
 ...
 * Alias for isSignedIn for consistency with earn app
 */
export const checkAuthStatus = isSignedIn;

/**
 * Get authentication token
 * Alias for getJWTToken for consistency with earn app
 */
