...
/** Simpler request format for API routes - optional fields use defaults when missing */
...
/** Paginated response from /api/autoramps */
...
/** Paginated response from /api/customers/{id}/virtual-accounts */
...
class IronFinanceClient {
   ...
   * Accepts either full SimplifiedBankAccountRequest or SimpleBankAccountRequest (with defaults).
   */
  async registerBankAccount(
    request: SimplifiedBankAccountRequest | SimpleBankAccountRequest,
    idempotencyKey?: string,
  ): Promise<FiatAddress> {
    // Normalize SimpleBankAccountRequest to SimplifiedBankAccountRequest
