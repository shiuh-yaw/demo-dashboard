/**
 * TransactionRecordService + WebhookEventService contracts, exercised
 * against their Postgres implementations backed by an in-memory fake of
 * `prisma.transaction`.
 *
 * State validation comes from `@dynamic-demos/transactions`;
 * `PostgresTransactionRecordService` must call `assertValidTransition` at
 * the boundary before every state mutation.
 */
