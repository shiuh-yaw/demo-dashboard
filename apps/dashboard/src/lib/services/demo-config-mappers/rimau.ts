/**
 * Rimau ↔ DemoConfig mapper. Same shape as trade: branding in the row, theme hydrated from the bound Prospect.
...
function mergeConfig(
  base: RimauConfig,
  partial: Partial<RimauConfig> | undefined,
): RimauConfig {
  const merged: RimauConfig = {
...
export const rimauMapper: DemoConfigMapper<RimauConfig, StoredRimauConfig> = {
  kind: "rimau",
  untitledLabel: "Untitled Rimau Config",

  async toCreateInput(_prospects, input) {
    const merged = mergeConfig(DEFAULT_RIMAU_CONFIG, input.config);
    return {
      kind: rimauMapper.kind,
  ...
  async toUpdateInput(_prospects, existing, input) {
    const existingConfig = existing.config as RimauConfig;
  ...
  toStored(record, prospect) {
    const config = record.config as RimauConfig | null | undefined;
    ...
    return {
      id: record.id,
      name: record.name ?? rimauMapper.untitledLabel,
