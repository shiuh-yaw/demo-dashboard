...
class LiFiService {
   ...
   * Uses /quote/toAmount endpoint for reverse quotes:
   * "Merchant wants to receive X tokens, how much does user need to send?"
   */
  async getQuote(
    ...
    // Using /quote/toAmount endpoint for reverse quote (merchant wants exact destination amount)
    ...
    const url = `${this.apiUrl}/quote/toAmount?${params.toString()}`;
