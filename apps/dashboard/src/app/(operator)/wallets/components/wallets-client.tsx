...
/**
 * Client-side Wallets Dashboard Component
 *
...
export function WalletsClient({
  ...
  async function handleDelete() {
    ...
    try {
      setIsDeleting(true);
      const result = await deleteWalletConfig(showDeleteModal.id);
      ...
      if (result.success) {
        ...
        setToast("Wallet config deleted");
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-xl font-semibold text-slate-900">Wallets</h1>
          ...
          <div className="divide-y divide-slate-100">
            {displayedConfigs.map((config) => (
              <div
                ...
                role="button"
                tabIndex={0}
                onClick={() => router.push(`/wallets/${config.id}`)}
                onKeyDown={(event) => {
