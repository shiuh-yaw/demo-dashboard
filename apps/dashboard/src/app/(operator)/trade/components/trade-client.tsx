...
/**
 * Client-side Trade Dashboard Component
 *
...
import { Plus, Settings, Trash2, ExternalLink, LineChart } from "lucide-react";
...
export function TradeClient({
  ...
  async function handleDelete() {
    ...
    try {
      setIsDeleting(true);
      const result = await deleteTradeConfig(showDeleteModal.id);
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-xl font-semibold text-slate-900">Trade</h1>
      ...
      {displayedConfigs.length === 0 && (
        <div className="bg-white rounded-xl border border-slate-200 p-8 text-center">
          ...
          <h3 className="text-sm font-semibold text-slate-900 mb-1.5">
            No Trade configs yet
          </h3>
          <p className="text-xs text-slate-500 mb-4 max-w-sm mx-auto">
            Create your first Trade config to customize the theme and
            branding.
          </p>
          ...
          <div className="divide-y divide-slate-100">
            {displayedConfigs.map((config) => (
              <div
                ...
                role="button"
                tabIndex={0}
                onClick={() => router.push(`/trade/${config.id}`)}
                onKeyDown={(event) => {
                  if (event.key === "Enter" || event.key === " ") {
                    event.preventDefault();
                    router.push(`/trade/${config.id}`);
                  }
                }}
              >
                ...
                <div className="px-4 py-3 flex items-center justify-end gap-0.5">
                  ...
                  <Tooltip content="Open Demo" position="top">
                    <a
                      href={demoThemeUrl("trade", config.id)}
                      target="_blank"
