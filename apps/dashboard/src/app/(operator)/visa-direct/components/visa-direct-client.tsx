...
/**
 * Client-side Visa Direct Dashboard Component
 *
...
import { Plus, Settings, Trash2, ExternalLink, CreditCard } from "lucide-react";
...
export function VisaDirectClient({
  initialConfigs,
  orphanedConfigs: initialOrphaned,
  currentUserId,
}: VisaDirectClientProps) {
  ...
  async function handleDelete() {
    ...
    try {
      setIsDeleting(true);
      const result = await deleteVisaDirectConfig(showDeleteModal.id);
      ...
      if (result.success) {
        ...
        setToast("Visa Direct config deleted");
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-xl font-semibold text-slate-900">Visa Direct</h1>
      ...
      {displayedConfigs.length === 0 && (
        <div className="bg-white rounded-xl border border-slate-200 p-8 text-center">
          ...
          <h3 className="text-sm font-semibold text-slate-900 mb-1.5">
            No Visa Direct configs yet
          </h3>
          <p className="text-xs text-slate-500 mb-4 max-w-sm mx-auto">
            Create your first Visa Direct config to customize the theme and
            branding.
          </p>
          ...
          <div className="divide-y divide-slate-100">
            {displayedConfigs.map((config) => (
              <div
                ...
                role="button"
                tabIndex={0}
                onClick={() => router.push(`/visa-direct/${config.id}`)}
                onKeyDown={(event) => {
                  if (event.key === "Enter" || event.key === " ") {
                    event.preventDefault();
                    router.push(`/visa-direct/${config.id}`);
                  }
                }}
              >
                ...
                <div className="px-4 py-3 flex items-center justify-end gap-0.5">
                  ...
                  <Tooltip content="Open Demo" position="top">
                    <a
                      href={demoThemeUrl("visa-direct", config.id)}
                      target="_blank"
      ...
      <ConfirmModal
        ...
        title="Delete Visa Direct Config"
        description={
          <>
