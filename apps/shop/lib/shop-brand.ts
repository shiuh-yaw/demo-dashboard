 ...
 *
 * Only the tokens shop personalizes per brand are emitted; everything
 * else falls through to the static `--brand-*` defaults in
 * `app/globals.css` (which encode shop's own design language) and to
 * `@dynamic-demos/theme/defaults.css` below that. Zero FOUC,
 * zero hydration mismatch — the inline `<style>` beats client paint.
 *
 * Mirrors `apps/wallet/lib/wallet-brand.ts` — keep the two helpers in
 * sync until the shared abstraction lands in `@dynamic-demos/theme`.
 */
