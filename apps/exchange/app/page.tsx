/**
 * Exchange scenario page (Server Component) - the demo's front door and its
...
import { buildCodeSteps, EXCHANGE_SDK_STEPS } from "@/lib/code-steps";

export default async function HomePage() {
  const { isBranded } = await getExchangeConfig();
  const chrome = buildScenarioChrome({
    chip: "Exchange",
    isBranded,
    brandLogo: <ScenarioBrandLogo />,
  });
  const sdkSteps = await buildCodeSteps(EXCHANGE_SDK_STEPS);
