...
**Confidential — Not for onward distribution.** APAC Sales Engineering demo (Brief 04). Exchange is a fictional client.
...
```bash
pnpm dev:exchange            # http://localhost:4015 - staged mode when no env id is set
```
...
Then in `apps/exchange/.env`:

```bash
NEXT_PUBLIC_DYNAMIC_ENVIRONMENT_ID=<sandbox environment id>
NEXT_PUBLIC_EXCHANGE_MODE=live            # optional; live is inferred from the id
