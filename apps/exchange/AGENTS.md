...
- Hosting: `Dockerfile` + `railway.json` build from the repo root without a JFrog token (README "Deploy to Railway"); `NEXT_PUBLIC_*` are build args. Vercel still works via `vercel.json`.
