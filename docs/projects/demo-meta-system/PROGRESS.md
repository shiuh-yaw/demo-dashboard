...
| 2-migrate. Unified `DemoConfig` table | 🟡 in-flight | [#81](https://github.com/dynamic-labs/demo-dashboard/pull/81), [#82](https://github.com/dynamic-labs/demo-dashboard/pull/82) | Single `DemoConfig` Postgres table with `kind` discriminator replaces what would have been one table per demo type — see D-029. PR #81 covers earn, wallet, trade, visa-direct, checkout. PR #82 folds RemittanceConfig into the unified table (`kind="remittance"`), drops the legacy table, and extends `backfill:demo-configs` to walk the remittance Redis store. The previously-planned per-type PRs (#83/#84/#85 in the original layout) are obsolete and will be closed. |
| 2-migrate. Action-layer cutover (TD-002) | 🟡 in-flight | [#83](https://github.com/dynamic-labs/demo-dashboard/pull/83) | All 6 demo-type action files (`earns`, `wallets`, `trade`, `visa-direct`, `checkouts`, `remittance`) now route through `services.demoConfigs.*` via per-kind mappers in `lib/services/demo-config-mappers/`. Brand resolution is deterministic — `(ownerId, primaryColor, logoUrl)` hashes via `scripts/backfill-brands/hash.ts` so action-created and backfill-created rows converge on the same Brand. `RedisDemoConfigService.get` falls back to the legacy per-kind keyspace (read-only, no lazy upsert) so the cutover is read-non-breaking. `USE_POSTGRES_DEMO_CONFIGS=false` default preserved. |
