...
export const GET = withAuth(
  async (req: NextRequest, { params }: { params: PaymentParams }) => {
    try {
      ...
      const payment = await ironClient.getThirdPartyPayment(id);
      return createResponse(payment, 200, req);
    } catch (error) {
      return handleApiError(error, "iron/third-party-payments/get", req);
    }
  }
);
