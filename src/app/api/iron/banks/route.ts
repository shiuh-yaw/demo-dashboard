...
import { ironClient, type RegisterBankAccountRequest } from "@/lib/services/iron";
import { z } from "zod";
...
export const POST = withAuth(async (req: NextRequest) => {
  try {
    ...
    const bankRequest: RegisterBankAccountRequest = {
    ...
    return createResponse(bankAccount, 201, req);
  } catch (error) {
    return handleApiError(error, "iron/banks/create", req);
  }
});
