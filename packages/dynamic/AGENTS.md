...
- `renderDemoOgImage` (`./og-image`) — generic OG/Twitter unfurl image composer built on `next/og`'s `ImageResponse`. Prefer `{ slug, art }`: the demo's name and its short pitch are then read from `DEMO_CATALOG`, so the unfurl, the landing card and the nav grid cannot say three different things. `{ demoLabel }` remains for the four apps with no catalog entry (deposit, proceeds, shop, cross-border AP/AR) and falls back to a generic subtitle; passing neither throws. Still no prospect/brand/theme data and no config fetch - catalog copy is product copy, so branded and bare URLs unfurl identically. Unlike `noindex`, which only blocks search crawlers, this is what stops a forwarded branded link's *preview* from leaking which customer it is for. Renders the landing's category-tinted hero band full-bleed at 1200x630 (`DemoHeroBand` + the shared illustration for `art`), the full Dynamic lockup including "a Fireblocks company" (outlines from `@dynamic-demos/ui/dynamic-logo-paths`, never `<text>`), the name, the pitch, and the `demo.dynamic.xyz` footer. **Editing this means satisfying satori, whose failure mode for an unsupported construct is a SILENT no-op.** Verified: `<text>` throws; component elements inside `<svg>` render nothing; fragments inside `<svg>` throw; `background-size` dot tiling is ignored (the grid is real `<circle>`s); gradients, `fillOpacity`, `strokeDasharray`, `box-shadow` and inline `style` on SVG children all work; a bare color inside the `background` shorthand throws (set `backgroundColor` and `backgroundImage` separately). Font loading best-effort fetches DM Sans - the brand face - from Google Fonts, subset to the glyphs it renders, and silently falls back to next/og's bundled font on any failure, so an OG request never 500s. The accepted `src` formats are ttf/otf/**woff**: Google serves DM Sans as woff, and matching only truetype/opentype is what silently dropped the brand font before. The og-image test asserts every motif changes the rendered pixels, which is what catches a silent drop.
...
- `@dynamic-demos/dynamic/networks` — `createNetworkConfig` + chain ids, and `deriveChainOptions(networks, { only })` which groups live `getNetworksData()` entries into one `ChainOption` row per chain family (icon from the family's first network, `only` both filters and fixes the order so the list doesn't reshuffle when the environment reorders). Pure and SDK-free by design: it takes a structural `NetworkLike`, never an imported `NetworkData`, because apps do not agree on an SDK version (accounts pins 1.25.0, the catalog is 0.25.0) and importing one here would bind every consumer to it. Each app keeps a thin hook that fetches with its own SDK — see `apps/accounts` and `apps/wallet` `hooks/use-chain-options.ts`.
...
- `@dynamic-demos/dynamic/og-image` — `renderDemoOgImage`, `buildOgFontSubsetText`, `OgArtKey` (`wallet | connect | accounts | trade | earn | checkout | transfer | card`), `OG_IMAGE_WIDTH`, `OG_IMAGE_HEIGHT`. Not re-exported from the root barrel (pulls in `next/og`'s renderer) - always import this subpath directly.
