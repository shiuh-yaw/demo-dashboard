...
- Scenario — `ScenarioHero`, `ScenarioEyebrow`, `RouteChip`, `ChipArrow`, `ScenarioLayout`, and the branded-hero primitives `ScenarioBrandRow` (logo left / `BookACallButton` right - owns the row spacing that stands in for the hidden SiteHeader under `?theme=`) + `ScenarioBrandImage` (aspect-normalized brand `<img>`, `align: start|center`, carries its own mb). Apps keep a thin config-reading `scenario-brand-logo.tsx` that delegates rendering here - never re-implement the row or image locally (spacing drift is how the earn/wallet headers diverged).
- Code panel — `CodePanel`, `Stepper`, `CodeFrame`, `DocsLink`, `renderProse`, `CodeStep`, `CodePanelProps`, `PanelNotice` (+ `PanelNoticeProps`) — flow's gradient callout shell, and `SdkStack` (+ `SdkStackProps`) — "Built with" package list on that shell with an optional docs link, typically passed via `CodePanel`'s `notice` slot. Scenario apps must also `@import "@dynamic-demos/ui/code-panel.css"` in their globals (shiki line numbers + scrollbar styling for the code frame; without it, code blocks render without line numbers).
