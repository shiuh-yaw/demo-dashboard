...
- Integration code panel: `CodePanel` (pill tabs, hash deep-links), `Stepper`, `CodeFrame`, `DocsLink`, `renderProse` — content arrives as pre-highlighted HTML (`CodeStep`); the package has no Shiki dependency. `renderProse` lives in its own non-"use client" module (`render-prose.tsx`) so server components can call it during render; do not move it back into the client atoms file.
