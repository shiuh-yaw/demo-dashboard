...
- `WalletPickerScreen` / `WalletPickerScreenProps` — installed + discovered (WalletConnect) wallet picker with QR surface, search, mobile deeplink fallback. Snapshots the prior wallet address so the parent only sees the new connection. `verifyOnConnect` (default `true`) toggles between `connectAndVerifyWithWalletProvider` (with SIWE challenge) and `connectWithWalletProvider` (no signature) for both installed and WalletConnect paths. Multi-chain wallets (e.g. Phantom EVM + SOL) trigger a chain selection sub-view via `selectedWalletForChain` / `onChainSelectChange` props — the user picks which chain to connect before the handshake begins (mirrors `apps/checkouts` pattern). Single-chain wallets connect directly. (stable)
